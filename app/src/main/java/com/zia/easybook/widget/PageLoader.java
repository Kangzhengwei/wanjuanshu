...
import com.zia.easybook.utils.ScreenUtils;
import com.zia.easybook.utils.StringUtils;
...
public abstract class PageLoader {
    ...
    // 当前章
    int mCurChapterPos;//章节指针
    int mCurPagePos;//页数指针
    ...
    /*****************************init params*******************************/
    PageLoader(PageView pageView, Book book, Catalog catalog, Callback callback) {
        mPageView = pageView;
        this.book = book;
        ...
        //catalog = SPUtils.getObject(mContext, "catalog" + book.getBookName() + book.getAuthor(), Catalog.class);
        mCurChapterPos = catalog.getIndex();
        mCurPagePos = catalog.getDurChapterPage();
    ...
    public boolean skipNextChapter() {
        if (mCurChapterPos + 1 >= Integer.valueOf(book.getChapterSize())) {
            return false;
        }
     ...
     * 获取章节的文本
     */
    protected abstract Observer<List<String>> getChapterContent(Catalog chapter) throws Exception;

    /**
     * 章节数据是否存在
     */
    protected abstract boolean noChapterData(Catalog chapter);
    ...
    void pagingEnd(PageAnimation.Direction direction) {
        ...
        switch (direction) {
            case NEXT:
                ...
                } else if (mCurChapterPos < Integer.valueOf(book.getChapterSize()) - 1) {
                    mCurChapterPos = mCurChapterPos + 1;
                    mCurPagePos = 0;
                    Collections.swap(chapterContainers, 0, 1);
                    Collections.swap(chapterContainers, 1, 2);
        ...
        catalog.setIndex(mCurChapterPos);
        catalog.setDurChapterPage(mCurPagePos);
    ...
    private synchronized void drawBackground(final Canvas canvas, TxtChapter txtChapter, TxtPage txtPage) {
        ...
        if (!callback.getChapterList().isEmpty()) {
            ...
            String progress = (txtChapter.getStatus() != TxtChapter.Status.FINISH) ? "" : getReadProgress(mCurChapterPos, Integer.valueOf(book.getChapterSize()), mCurPagePos, curChapter().txtChapter.getPageSize());
    ...
    private synchronized void drawContent(Bitmap bitmap, TxtChapter txtChapter, TxtPage txtPage) {
        ...
        if (txtChapter.getStatus() != TxtChapter.Status.FINISH) {
            ...
            for (int i = 0; i < txtPage.getTitleLines(); ++i) {
                ...
                //mTitlePaint.setColor(ThemeStore.accentColor(mContext));
            ...
            for (int i = txtPage.getTitleLines(); i < txtPage.size(); ++i) {
               ...
               // mTextPaint.setColor(ThemeStore.accentColor(mContext));
    ...
    void drawContent(final Canvas canvas, float offset) {
        ...
        while (top < totalHeight) {
            ...
            if (pagePos == chapter.getPageSize() - 1) {
                String sign = "\u23af \u23af";
                if (chapterPos == Integer.valueOf(book.getChapterSize()) - 1) {
    ...
    boolean hasNext(int pageOnCur) {
        ...
        return mCurChapterPos + 1 < Integer.valueOf(book.getChapterSize());
    }
    ...
    public interface Callback {
         ...
         */
        void onCategoryFinish(List<Catalog> chapters);
    ...
    public String getReadProgress(int durChapterIndex, int chapterAll, int durPageIndex, int durPageAll) {
